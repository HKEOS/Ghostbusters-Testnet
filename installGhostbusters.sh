...
if [[ ! -d $TESTNET_DIR ]]; then
    ...
    echo -ne "\n"
    if [[ $NODE_SSL_PORT != "" ]]; then
        echo "\$CLEOS -u https://$NODE_HTTP_SRV_ADDR --wallet-url http://127.0.0.1:$WALLET_PORT \"\$@\"" >> $TESTNET_DIR/cleos.sh
    else
        echo "\$CLEOS -u http://$NODE_HTTP_SRV_ADDR --wallet-url http://127.0.0.1:$WALLET_PORT \"\$@\"" >> $TESTNET_DIR/cleos.sh
...
if [[ $ISBP == true ]]; then
    echo 'signature-provider = '$PRODUCER_PUB_KEY'=KEY:'$PRODUCER_PRIV_KEY'' >> $BASE_CONFIG;
    ...
    echo "plugin = eosio::history_api_plugin" >> $BASE_CONFIG;
fi
